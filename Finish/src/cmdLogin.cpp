...
void Server::handleLogin(int clientSocket, User& user)
{
    ...
    {
        std::cout << "[INFO] User " << user.getNick() << "@" << user.getName() << " disconnected due to duplicate nick!" << std::endl;
        if (Utils::IsSocketWritable(clientSocket))
    ...
    std::cout << "[INFO] User " << user.getNick() << "@" << user.getName() << " authentified !" << std::endl;
}
