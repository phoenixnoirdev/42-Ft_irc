...
void Server::handleKickCommand(int clientSocket, const std::string& line)
{
    ...
    if (chan.GetGradeUser(kicker) == 0 || chan.GetGradeUser(kicker) == 1 || chan.GetOpChannel() == kicker.getSocket())
    {
        ...
        {
            std::cout << RED << "[KICK]: " << chanName << " " << kicker.getName() << " trying to use the KICK command on himself " << targetNick << " = " << reason << std::endl;

            std::string note = ":" + this->_ServName + "You cannot kick yourself.\r\n";
            if (Utils::IsSocketWritable(clientSocket))
        ...
        if (!(chan.GetGradeUser(kicker) <= chan.GetGradeUser(target)))
        {
            ...
            {
                std::cout << RED << "[KICK]: " << chanName << " " << kicker.getName() << " tried to use the KICK command on the server operator user " << targetNick << " = " << reason << std::endl;

                std::string note = ":" + this->_ServName + "You cannot kick a server operator.\r\n";
                if (Utils::IsSocketWritable(clientSocket))
                    ::send(clientSocket, note.c_str(), note.size(), 0);
                return;
            }
            ...
            {   
                std::cout << RED << "[KICK]: " << chanName << " " << kicker.getName() << " tried to use the KICK command on the user with equal or higher grade " << targetNick << " = " << reason << std::endl;
      
                std::string note = ":" + this->_ServName + "You cannot kick a user with equal or higher grade.\r\n";
                if (Utils::IsSocketWritable(clientSocket))
                    ::send(clientSocket, note.c_str(), note.size(), 0);
                return;
            }
    ...
    else
    {
        ...
        std::cout << RED << "[KICK]: " << chanName << " " << kicker.getName() << " tried to use the KICK command on the user " << targetNick << " = " << reason << std::endl;
