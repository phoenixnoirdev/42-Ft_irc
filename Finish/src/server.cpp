...
void Server::Shutdown()
{
	...
	std::cout << YELLOW << "closing propre du serveur..." << RESET << std::endl;
	std::cout << YELLOW << "------------------------------------" << RESET << std::endl;
}
...
std::string Server::GetPwd(const std::string& str)
{
	if (str.substr(0, 4) == "PASS")
	{
		...
		if (spacePos != std::string::npos)
		{
			...
			if (DEBUG == true)
				std::cout << "[DEBUG] PASS finded: " << pass << std::endl;
...
std::string Server::GetNick(const std::string& str)
{
	if (str.substr(0, 4) == "NICK")
	{
		...
		if (spacePos != std::string::npos)
		{
			...
			if (DEBUG == true)
				std::cout << "[DEBUG] NICK finded: " << nick << std::endl;
	...
	if (auth == false)
	{
		if (str.substr(0, 4) == "USER")
		{
			...
			if (colonPos != std::string::npos)
			{
				...
				if (DEBUG == true)
					std::cout << "[DEBUG] USER finded INIT: " << user << std::endl;
...
void Server::handleOperCommand(int clientSocket, const std::string& line)
{
    ...
    if (password == _AdminConfig.getOperPassword())
    {
        ...
        if (currentGrade == 0)
        {
            std::string msg1 = ":" + _ServName + " NOTICE " + nickname + " :👑 You are already the PRINCIPAL ADMINISTRATOR!\r\n";
            if (Utils::IsSocketWritable(clientSocket))
                send(clientSocket, msg1.c_str(), msg1.length(), 0);
            
            std::string msg2 = ":" + _ServName + " NOTICE " + nickname + " :⚡ Grade 0 is higher than Grade 1 (Operator)\r\n";
            if (Utils::IsSocketWritable(clientSocket))
        ...
        else
        {
            ...
            std::string successMsg1 = ":" + _ServName + " NOTICE " + nickname + " :🎉 PROMOTION SUCCESSFUL!\r\n";
            if (Utils::IsSocketWritable(clientSocket))
                send(clientSocket, successMsg1.c_str(), successMsg1.length(), 0);
            
            std::string successMsg2 = ":" + _ServName + " NOTICE " + nickname + " :⚡ You are now an OPERATOR (Grade 1)\r\n";
            if (Utils::IsSocketWritable(clientSocket))
                send(clientSocket, successMsg2.c_str(), successMsg2.length(), 0);
            
            std::string successMsg3 = ":" + _ServName + " NOTICE " + nickname + " :📋 You can use: MODE +o, KICK, BAN\r\n";
            if (Utils::IsSocketWritable(clientSocket))
    ...
    {
        std::string errorMsg = ":" + _ServName + " NOTICE " + nickname + " :❌ Incorrect password for OPER\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, errorMsg.c_str(), errorMsg.length(), 0);
...
void Server::sendWelcomeMessage(int clientSocket, const User& user)
{
    ...
    std::string welcomeMsg = ":" + _ServName + " 001 " + nickname + " :Welcome to " + _ServName + "!\r\n";
    if (Utils::IsSocketWritable(clientSocket))
    ...
    {
        case 0:
            statusMsg += "👑 PRINCIPAL ADMINISTRATOR (Grade 0)";
            break;
        case 1:
            statusMsg += "⚡ OPERATOR (Grade 1)";
            break;
        case 2:
            statusMsg += "🔊 VOICE (Grade 2)";
            break;
        case 3:
            statusMsg += "👤 NORMAL USER (Grade 3)";
            break;
        default:
            statusMsg += "❓ UNKNOWN";
    }
    ...
    {
        case 0:
            privMsg += "Full server control";
            break;
        case 1:
            privMsg += "Moderate channels, give OP, kick/ban";
            break;
        case 2:
            privMsg += "Speak in moderated channels (+m)";
            break;
        case 3:
            privMsg += "Basic channel access";
            break;
        default:
            privMsg += "None";
    }
    ...
    if (grade == 0)
    {
        std::string adminMsg1 = ":" + _ServName + " 005 " + nickname + " :🎯 You have FULL CONTROL of the server!\r\n";
        if (Utils::IsSocketWritable(clientSocket))
    ...
    {
        std::string operMsg1 = ":" + _ServName + " 005 " + nickname + " :⚡ You are OPERATOR of the server!\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, operMsg1.c_str(), operMsg1.length(), 0);
        
        std::string operMsg2 = ":" + _ServName + " 006 " + nickname + " :📋 Commands: MODE +o, KICK, BAN in channels\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, operMsg2.c_str(), operMsg2.length(), 0);
    }
    else if (grade == 2)
    {
        std::string voiceMsg = ":" + _ServName + " 005 " + nickname + " :🔊 You have VOICE - you can speak in moderated channels\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, voiceMsg.c_str(), voiceMsg.length(), 0);
    }
    else if (grade == 3)
    {
        std::string userMsg1 = ":" + _ServName + " 005 " + nickname + " :📈 To gain more privileges:\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, userMsg1.c_str(), userMsg1.length(), 0);
        
        std::string userMsg2 = ":" + _ServName + " 006 " + nickname + " :   💼 OPER " + nickname + " <password> → Become Operator\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, userMsg2.c_str(), userMsg2.length(), 0);
        
        if (nickname == _AdminConfig.getMainAdmin())
        {
            std::string adminHint = ":" + _ServName + " 007 " + nickname + " :   👑 ADMIN " + nickname + " <password> → Become Main Admin\r\n";
            if (Utils::IsSocketWritable(clientSocket))
    ...
    {
        case 0:
            visibleMsg += "👑 You are MAIN ADMIN (Grade 0) - Full control!";
            break;
        case 1:
            visibleMsg += "⚡ You are OPERATOR (Grade 1) - Can moderate channels";
            break;
        case 2:
            visibleMsg += "🔊 You have VOICE (Grade 2) - Can speak in +m channels";
            break;
        case 3:
            visibleMsg += "👤 Normal user (Grade 3) - Use OPER for privileges";
            break;
    }
    visibleMsg += "\r\n";
    if (Utils::IsSocketWritable(clientSocket))
...
void Server::handleAdminCommand(int clientSocket, const std::string& line)
{
    ...
    if (_AdminConfig.validateAdminCredentials(username, password))
    {
        ...
        std::string successMsg1 = ":" + _ServName + " NOTICE " + nickname + " :🎉 ADMIN AUTHENTICATION SUCCESSFUL!\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, successMsg1.c_str(), successMsg1.length(), 0);
        
        std::string successMsg2 = ":" + _ServName + " NOTICE " + nickname + " :👑 You are now the PRINCIPAL ADMINISTRATOR\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, successMsg2.c_str(), successMsg2.length(), 0);
        
        std::string successMsg3 = ":" + _ServName + " NOTICE " + nickname + " :⚡ Grade 0 - TOTAL SERVER CONTROL\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, successMsg3.c_str(), successMsg3.length(), 0);
        
        std::string successMsg4 = ":" + _ServName + " NOTICE " + nickname + " :📋 All commands available!\r\n";
        if (Utils::IsSocketWritable(clientSocket))
    ...
    {
        std::string errorMsg = ":" + _ServName + " NOTICE " + nickname + " :❌ Invalid admin credentials\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, errorMsg.c_str(), errorMsg.length(), 0);
...
{
    std::cout << "=====================================" << std::endl;
    std::cout << "📋 ADMINISTRATION SETTINGS" << std::endl;
    std::cout << "=====================================" << std::endl;
    std::cout << "🔑 Principal Administrator: " << _AdminConfig.getMainAdmin() << std::endl;
    std::cout << "   └─ Command: ADMIN " << _AdminConfig.getMainAdmin() << " " << _AdminConfig.getAdminPassword() << std::endl;
    std::cout << std::endl;
    std::cout << "👥 For Operators (Grade 1):" << std::endl;
    std::cout << "   └─ Command: OPER <nickname> " << _AdminConfig.getOperPassword() << std::endl;
    std::cout << std::endl;
    std::cout << "📊 System Grades:" << std::endl;
    std::cout << "   Grade 0: Principal Administrator (total control)" << std::endl;
    std::cout << "   Grade 1: Operator (channel moderation)" << std::endl;
    std::cout << "   Grade 2: Voice (speak in +m channels)" << std::endl;
    std::cout << "   Grade 3: Normal User" << std::endl;
    std::cout << "=====================================" << std::endl;
}
...
void Server::handleGradesCommand(int clientSocket, const std::string& /* line */)
{
    ...
    std::string header = ":" + _ServName + "!" + _ServName + "@" + _ServName + " PRIVMSG " + nickname + " :📊 GRADES OF CONNECTED USERS\r\n";
    if (Utils::IsSocketWritable(clientSocket))
    ...
    for (std::map<int, User>::iterator it = _User.begin(); it != _User.end(); ++it)
    {
        ...
        {
            case 0:
                gradeMsg += "👑 " + userNick + " - Principal Administrator (Grade 0)";
                break;
            case 1:
                gradeMsg += "⚡ " + userNick + " - Operator (Grade 1)";
                break;
            case 2:
                gradeMsg += "🔊 " + userNick + " - Voice (Grade 2)";
                break;
            case 3:
                gradeMsg += "👤 " + userNick + " - Normal User (Grade 3)";
                break;
            default:
                gradeMsg += "❓ " + userNick + " - Unknown";
        }
        gradeMsg += "\r\n";
        if (Utils::IsSocketWritable(clientSocket))
    ...
    std::string footer = ":" + _ServName + "!" + _ServName + "@" + _ServName + " PRIVMSG " + nickname + " :💡 Use: SETGRADE <nick> <0-3> to change\r\n";
    if (Utils::IsSocketWritable(clientSocket))
...
void Server::handleSetGradeCommand(int clientSocket, const std::string& line)
{
    ...
    if (tokens.size() < 3)
    {
        std::string errorMsg = ":" + _ServName + "!" + _ServName + "@" + _ServName + " PRIVMSG * :❌ Usage: SETGRADE <nickname> <grade>\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, errorMsg.c_str(), errorMsg.length(), 0);
        return;
    }
    ...
    if (adminGrade != 0)
    {
        std::string errorMsg = ":" + _ServName + "!" + _ServName + "@" + _ServName + " PRIVMSG " + adminNick + " :❌ Only the Administrator Principal (Grade 0) can use SETGRADE\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, errorMsg.c_str(), errorMsg.length(), 0);
        return;
    }
    ...
    {
        std::string errorMsg = ":" + _ServName + "!" + _ServName + "@" + _ServName + " PRIVMSG " + adminNick + " :❌ The grade must be 0, 1, 2, or 3\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, errorMsg.c_str(), errorMsg.length(), 0);
    ...
    for (std::map<int, User>::iterator it = _User.begin(); it != _User.end(); ++it)
    {
        if (it->second.getNick() == targetNick)
        {
            ...
            std::string successMsg = ":" + _ServName + "!" + _ServName + "@" + _ServName + " PRIVMSG " + adminNick + " :✅ " + targetNick + " modified from Grade " + oldGradeChar + " to Grade " + newGradeChar + "\r\n";
            if (Utils::IsSocketWritable(clientSocket))
            ...
            {
                case 0: 
                    gradeName = "👑 Administrator Principal";
                    break;
            ...
            std::string notifyMsg = ":" + _ServName + "!" + _ServName + "@" + _ServName + " PRIVMSG " + targetNick + " :🎉 Your grade has been changed to: " + gradeName + " (Grade " + newGradeChar + ")\r\n";
            if (Utils::IsSocketWritable(clientSocket))
    ...
    if (!found)
    {
        std::string errorMsg = ":" + _ServName + "!" + _ServName + "@" + _ServName + " PRIVMSG " + adminNick + " :❌ User '" + targetNick + "' not found\r\n";
        if (Utils::IsSocketWritable(clientSocket))
            send(clientSocket, errorMsg.c_str(), errorMsg.length(), 0);
    }
}
