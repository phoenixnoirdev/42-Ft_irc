...
void Server::handleBrodcastPrivateMsg(User& user, std::string line)
{
    ...
    {
        std::string ircMsg = ":" + user.getNick() + "!~" + user.getName() + "@localhost PRIVMSG " + user.getNick() + " :The recipient of your private message is not connected (" + msg + ")\r\n";
...
void Server::handleBrodcastMsgKB(User& user, std::string line)
{
    ...
    std::string ircMsg = ":" + user.getNick() + "!~" + user.getName() + "@localhost PRIVMSG " + user.getNick() + " : You have been kicked or banned. The message was not sent (" + msg + ")\r\n";
    if (Utils::IsSocketWritable(user.getSocket()))
        send(user.getSocket(), ircMsg.c_str(), ircMsg.size(), 0);
