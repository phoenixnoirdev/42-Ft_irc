...
void Server::handleQuit(int clientSocket, User& user, const std::string& line)
{
    ...
    if (c != '#' && c != '&' && c != '+' && c != '!')
    {
        ...
        std::cout << RED << "[PART]: " << user.getNick() << " trying to use the PART command to leave the channel: " << chanName << RESET << std::endl;
    ...
    if (existe == false)
    {
        std::cout << "[INFO] User " << user.getNick() << " tried to leave the channel " << chanName << " but the channel does not exist." << std::endl;
    ...
    else
    {
        ...
        std::cout << "[INFO] User " << user.getNick() << " left the channel " << chanIt->second.GetName() << std::endl;
        ...
        {
            std::cout << "[INFO] Deleting channel " << chanIt->second.GetName() << " because the channel is empty " << std::endl;
